...
const char *OCPP_CONFIGURATION =
    ...
    "{\"key\":\"StopTransactionOnEVSideDisconnect\",\"readonly\":true,\"value\":\"false\"},"
    ...
    "{\"key\":\"SupportedFeatureProfiles\",\"readonly\":true,\"value\":\"Core,RemoteTrigger\"},"
...
void ocpp_handle_response_stop_transaction(const char *id, const char *payload) {
...
void ocpp_handle_ocpp_response(struct mg_connection *nc, const char *id, const char *payload) {
  if (strcmp(id, start_transaction_uuid) == 0) {
    ...
    ocpp_handle_response_stop_transaction(id, payload);
